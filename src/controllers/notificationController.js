...
const notificationController = {
  ...
  async markRead(req, res) {
    try {
      ...
      await notificationService.markRead(io, req.user.id, id);
  ...
  async markAllRead(req, res) {
    try {
      const io = req.app.get('io');
      await notificationService.markAllRead(io, req.user.id);
  ...
  async remove(req, res) {
    try {
      ...
      const io = req.app.get('io');
      await notificationService.remove(io, req.user.id, id);
      ...
      res.status(status).json({
        error: error.statusCode ? error.message : 'Failed to delete notification',
      });
    }
  },
};
