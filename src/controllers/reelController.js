...
const reelController = {
    ...
    try {
      ...
      (async () => {
        ...
        try {
          ...
          // Kind 2 notification (plan §6) — only fires when the reel is
          // published, per plan §6's event table. Fire-and-forget so a
          // slow fan-out to followers never blocks reel processing.
          if (isPublished) {
            const io = req.app.get('io');
            notificationService
              .notifyNewReel(io, pendingReel.id)
              .catch((notifyErr) =>
                console.error(
                  `❌ Failed to notify followers of reel ${pendingReel.id}:`,
                  notifyErr
                )
              );
          }
  ...
  async handleGetReelsByCategory( req,  res  ) {
    ...
    try {
      ...
      // "Following" feed (follow-feature plan §4) — same path, a
      // different query mode. Reels from players the viewer follows,
      // newest first, paginated.
      if (String(req.query.following).toLowerCase() === 'true') {
        if (!viewerId) {
          return res.status(401).json({
            success: false,
            message: 'Authentication required for the following feed.',
          });
        }
        ...
        const result = await getFollowingFeed(viewerId, { page, limit });
