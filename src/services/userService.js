...
// Follow-feature plan §5: which of `candidateIds` does viewerId already
// follow. Queried directly here (rather than importing followerService)
// to avoid a cross-service coupling for one small lookup.
async function getViewerFollowingSet(viewerId, candidateIds) {
...
const userService = {
  ...
  // PLAYERS
  // ===========================
  // viewerId is optional — pass req.user?.id so each player carries
  // viewerActions.isFollowing (follow-feature plan §5) without a
  // per-row follow-status request from the client.
  ...
  //   // Top-level viewerActions.isFollowing per plan §5.
  ...
  //       scoutFollowers: player._count.scoutFollowers,
