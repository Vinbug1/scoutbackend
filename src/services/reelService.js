...
// =========================================================
// 🔹 Attach the viewer's follow status onto each reel's player object
//    (follow-feature plan §5 — avoids a follow-status request per row)
// =========================================================
const attachPlayerFollowState = async (reels, viewerId) => {
  if (reels.length === 0) return reels;

  const playerIds = [...new Set(reels.map((r) => r.player?.id).filter(Boolean))];
  let followingSet = new Set();

  if (viewerId && playerIds.length) {
    const rows = await prisma.follower.findMany({
      where: { followerId: viewerId, followedId: { in: playerIds } },
      select: { followedId: true },
    });
    followingSet = new Set(rows.map((r) => r.followedId));
  }

  return reels.map((r) =>
    r.player
      ? { ...r, player: { ...r.player, viewerActions: { isFollowing: followingSet.has(r.player.id) } } }
      : r
  );
};
...
export const getReelsByUser = async (playerId, categoryId = null, viewerId = null) => {
  ...
  const withViewerActions = await attachViewerInteractions(formatted, viewerId);
  return attachPlayerFollowState(withViewerActions, viewerId);
};
...
export const getReelById = async (reelId, viewerId = null, ipHash = null) => {
  ...
  const [withViewerActions] = await attachViewerInteractions([formatReel(reel)], viewerId);
  const [result] = await attachPlayerFollowState([withViewerActions], viewerId);
  return result;
};
...
// =========================================================
// 🔹 "Following" feed — published reels from players the viewer follows
//    (follow-feature plan §4: GET /reels?following=true&page=&limit=).
//    Newest first, paginated — this is what makes follow useful to
//    someone who has push notifications switched off.
// =========================================================
export const getFollowingFeed = async (viewerId, { page = 1, limit = 20 } = {}) => {
  const skip = (page - 1) * limit;

  const followedRows = await prisma.follower.findMany({
  ...
  const where = { playerId: { in: followedIds }, published: true };
  ...
  const formatted = reels.map(formatReel);
  const withViewerActions = await attachViewerInteractions(formatted, viewerId);
  // Every reel here is already from a followed player, but this keeps
  // the player object's shape identical to every other reel endpoint.
  const withFollowState = await attachPlayerFollowState(withViewerActions, viewerId);

  return {
    data: withFollowState,
