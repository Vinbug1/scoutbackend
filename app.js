...
// CHANGED — followerRouters used to be mounted at `${api}/followers`
// (`/api/followers/:id/follow`), but the follow-feature plan and the
// mobile app both call `/users/:id/follow`. Mounting a second router
// under the same `/users` prefix is fine — Express tries userRouters'
// routes first, falls through to followerRouters for anything it
// doesn't match (e.g. `/users/:id/follow`, which userRouters has no
// route for).
app.use(`${api}/users`, followerRouters);
...
app.use(`${api}/pushTokens`, pushTokenRouters);
